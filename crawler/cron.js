...
import { checkProductUpdates } from './sources/products.js';
import { checkMarketUpdates } from './sources/market-trends.js';
...
const onlyPromotions = args.includes('promotions') || args.length === 0;
const onlyDividend = args.includes('dividend') || args.length === 0;
const onlyRatings = args.includes('ratings') || args.length === 0;
const onlyProducts = args.includes('products') || args.length === 0;
const onlyMarket = args.includes('market') || args.length === 0;

console.log('╔══════════════════════════════════╗');
console.log('║  香港保险数据爬虫 v1.3         ║');
...
try {
  ...
  // ── 4. 产品变更检测 ──────────────────────────────
  if (onlyProducts) {
    console.log('▸ 任务 4/5: 产品变更检测');
    results.products = await checkProductUpdates({ dryRun });
    console.log('');
  }

  // ── 5. 市场趋势 ──────────────────────────────────
  if (onlyMarket) {
    console.log('▸ 任务 5/5: 市场趋势更新');
    results.market = await checkMarketUpdates({ dryRun });
    console.log('');
  }
} catch (err) {
  console.error('\n❌ 爬虫运行异常:', err.message);
